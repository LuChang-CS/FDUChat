...
import com.fudan.im.DataBase.DBPool;
import com.fudan.im.DataBase.FriendDao;
import com.fudan.im.DataBase.MomentDao;
import com.fudan.im.DataBase.SaveMsgDao;
...
import com.fudan.im.server.ServerListen;
...
public class ClientActivity {
	...
	public void regist(TranObject tran) {
        ...
        public void changeUserInfo(TranObject tran) {
            User user = (User) tran.getObject();
            int res = UserDao.updateInfo(user);
            if (res == -1) {
                tran.setResult(Result.UPDATE_FAILED);
            } else {
                tran.setResult(Result.UPDATE_SUCCESS);
            }
            System.out.println("发送更新结果...");
            send(tran);
            System.out.println("success update");
        }
        
        public void pushMoment(TranObject tran) {
            int userId = (int) tran.getObject();
            List<Moment> res = MomentDao.getMomentsByUserId(userId);
            if (res == null) {
                tran.setResult(Result.PULL_MOMENT_FAILD);
            } else {
                tran.setResult(Result.PULL_MOMENT_SUCCESS);
            }
            tran.setObject(res);
            send(tran);
        }
	...
	public void sendMessage(TranObject tran) {
        ...
        public void sendMoment(TranObject tran) {
            Moment moment = (Moment) tran.getObject();
            int res = MomentDao.insertMoment(moment);
            if (res == -1) {
                tran.setResult(Result.SEND_MOMENT_FAILED);
            } else {
                tran.setResult(Result.SEND_MOMENT_SUCCESS);
            }
            send(tran);
        }
